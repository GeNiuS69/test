class User < ActiveRecord::Base
  ...
  attr_accessible :id, :is_active, :email, :password, :password_confirmation,
                  :username, :agree_to_terms, :location, :image, :screen_name,
                  :bio, :remove_image, :image_cache, :gender, :dob,
                  :total_points, :confirmed_at, :sign_in_count, :roles,
                  :current_sign_in_at, :app_role_ids, :institution_id,
                  :is_verified, :artist_image, :artist_bio,
                  :remove_artist_image, :artist_image_cache, :slug,
                  :total_flagged_count, :current_flagged_count, :suspended_at,
                  :suspended, :deleted, :deleted_at, :featured, :beats_count,
                  :tracks_count, :last_sign_in_at, :followers_count,
                  :followed_users_count, :remote_image_url, :confirmation_token,
                  :reposts_count, :updated_at, :newsletter_subscription,
                  :social_accounts
  ...
  ADMIN_EMAILS = ['vibhor@trantorinc.com', 'rahul@speazie.com',
                  'sahil@speazie.com', 'b.wichmann@artemia.com',
                  'j.ogan@artemia.com', 'k.silva@artemia.com'].freeze
  ...
  validates :agree_to_terms,
            acceptance: { message: 'Please accept Terms and Conditions' },
            on: :create
  ...
  has_many :unscoped_followed_users, -> { for_user_unscoped },
  ...
  has_many :unscoped_followers, -> { for_user_unscoped },
  ...
  has_many :points_histories, -> { where target_type: 'User' },
  ...
  has_many :mentions, -> { where mentionable_type: 'User' },
           class_name: 'Mention', foreign_key: :mentionable_id
  ...
  has_many :my_contests, class_name: 'Contest', foreign_key: 'owner_id',
                         inverse_of: :owner, dependent: :destroy
  has_many :reposts, dependent: :destroy
  has_many :reposted_songs, -> { audio_default_scope }, through: :reposts,
                                                        source: :audio,
                                                        class_name: 'V1::Audio',
                                                        dependent: :destroy
  ...
  scope :for_user_unscoped, -> { unscope(where: %i[is_active deleted suspended]) }
  ...
  def update_sign_details(suspended = false)
    if suspended
      update(sign_in_count: sign_in_count + 1, current_sign_in_at: Time.now,
             suspended: false, suspended_at: nil)
      show_user_data
    else
      update(sign_in_count: sign_in_count + 1, current_sign_in_at: Time.now)
    end
  ...
  def recommended_users
    ...
    users = %i[leaderboard_toppers trending_users].inject([]) do |tmp_users, strategy|
      tmp_users += send(strategy, (tmp_users.collect(&:id) + following_ids + [id]))
                   .sample(RECOMMENDED_USERS_COUNT)
    end.flatten
  ...
  def search_users(search)
    %i[following_users friends_friends default].inject([]) do |except_users, strategy|
      except_users += send(strategy, search, (except_users.collect(&:id) << id))
    end.flatten
  end
  ...
  def filter_my_tracks_beats(params)
    songs_type = if params[:type].present?
                   ...
                   if beats_count.zero?
                     if tracks_count.zero? & reposts_count > 0
                       'Repost'
    ...
    when 'Repost'
      ...
      beats
    end.includes(:likes, :user).order(id: :desc).page(params[:page])
  end
  ...
  def trace_user(user_agent, location, ip_address)
    Warden::Manager.after_set_user except: :fetch do
      user_tracing = user_tracings.where(ip_address: ip_address,
      ...
      if user_tracing
        ...
        user_tracing.save_attributes(ip_address, current_sign_in_at,
                                     user_agent, location_details)
      end
    end
  end
  ...
  def update_traced_user(user_agent, ip_address)
    ...
    Warden::Manager.before_logout do
      user_tracing = user_tracings.where(user_agent: user_agent,
  ...
  def serialize_user
    serializable_hash(only: %i[id username gender])
      .merge(image: { medium: proxy_image_url,
                      thumb: proxy_image_url('thumb') })
  end
  ...
  def update_points
    ...
    PointsHistory.create_points_history(total_points,
                                        POINT_ACTIONS[:user_create],
                                        id, USER_CREATION_POINTS,
                                        self.class.name)
  end
  ...
  def strip_whitespace
    ...
    screen_name.strip! if screen_name
  ...
  def mixpanel_registration(provider = 'email')
    MixpanelWorker.perform_async(user_id: id,
                                 event: REG_MIXPANEL_EVENT,
                                 provider: provider)
  end
  ...
  def update_deactivate_user
    return unless changed.include?('is_active')
    if is_active?
      show_user_data
    else
      hide_user_data
  ...
  def destroy
    run_callbacks :destroy do
      ...
      update(deleted: 1, deleted_at: Time.now, password: tmp_password,
  ...
  class << self
    def proxy_image_url(user_id, format = 'medium', _api = true, _version = 'v2')
      Rails.application.routes.url_helpers.image_api_user_url(id: user_id,
                                                              type: format)
    end
    ...
    def update_points(user_id, points, action, action_user, force_update = false)
      ...
      PointsHistory.create_points_history(user.total_points, action, user.id,
                                          points, user.class.name)
    end
    ...
    def user_detail(user)
      ...
      user_detail[:image] = { medium: user.proxy_image_url,
                              thumb: user.proxy_image_url('thumb') }
      ...
      user_detail[:institution] = { name: '', bg_color: '', text_color: '' } if user.institution
      user_detail
    end
    ...
    def find_for_database_authentication(options = {})
      where(['lower(username) = :value OR lower(email) = :value',
             { value: options[:login].try(:downcase) }]).first
    end

    def build_searching_order(relationships)
      orders = %i[friends_friend following]
      order_clause = 'CASE'
      leaders_id = relationships.map(&:followed_id)
      second_hop_leaders_id = Relationship.where(follower_id: leaders_id)
                                          .pluck(:followed_id).uniq
      second_hop_leaders_id -= leaders_id
      orders.each_with_index do |priority, index|
        if priority == :following
          order_clause << sanitize_sql_array([' WHEN users.id in (?) THEN ? ',
                                              leaders_id, index])
        elsif priority == :friends_friend
          order_clause << sanitize_sql_array([' WHEN users.id in (?) THEN ? ',
                                              second_hop_leaders_id, index])
        end
      end
